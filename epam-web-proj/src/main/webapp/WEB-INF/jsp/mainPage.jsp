...
<header>
        ...
        <fmt:setBundle basename="locale" var="loc" scope="session" />
            ...
            <fmt:message bundle="${loc}" key="showAnalysisForNurse" var="showAnalysisForNurse" />
            <fmt:message bundle="${loc}" key="showTreatmentForNurse" var="showTreatmentForNurse" />
            <fmt:message bundle="${loc}" key="noPatientsFound" var="noPatientsFound" />
...
<nav class="navbar navbar-light bg-light justify-content-between">
    ...
    <form class="form-inline" action = "MyController" method ="get">
          ...
          <input type = "text" name="surname" minlength="1" required placeholder= "${surnameSearch}" / >
          <input type= "submit" value = "${search}"/>
    </form>
    <c:if test="${sessionScope.role eq 'dr'}">
    <form class="form-inline" action = "MyController" method ="get">
    ...
    <c:if test="${sessionScope.role eq 'nurse'}">

        <form class="form-inline" action = "MyController" method ="get">
              <input type= "hidden" name="command" value="SHOW_DEPARTMENT_ANALYSIS" />
              <input type= "date" name="date" value="" />
              <input type= "submit" value = "${showAnalysisForNurse}"/>
        </form>

        <form class="form-inline" action = "MyController" method ="get">
              <input type= "hidden" name="command" value="SHOW_DEPARTMENT_TREATMENT" />
              <input type= "date" name="date" value="" />
              <input type= "submit" value = "${showTreatmentForNurse}"/>
        </form>
    ...
    <a class="btn btn-primary" href ="MyController?command=LOG_OUT">LogOut</a>
        ...
        <form align="right" action = "MyController" method ="get">
        ...
        <c:if test="${not empty requestScope.foundAnalysis}">
                <style type="text/css">
                   TABLE {
                   ...
                   <table align="center">
                   <tr><th>PatientId</th><th><c:out value="${surname}"/></th><th><c:out value="${name}"/>
                   </th><th><c:out value="${appointmentDate}"/></th><th><c:out value="${type}"/>
                   </th><th><c:out value="${room}"/></th><th></th></tr>
                           <c:forEach var="analysis" items="${foundAnalysis}">
                           <tr><td>${analysis.patientId}</td><td>${analysis.patientSurname}</td><td>${analysis.patientName}</td>
                           <td>${analysis.appointmentDate}</td><td>${analysis.type}</td><td>${patient.roomNumber}</td></tr>
     ...
     <c:if test="${not empty found and found eq 'false'}">
